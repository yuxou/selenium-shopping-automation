...
> **주의**: 이 코드는 학습 및 테스트 목적으로 작성되었습니다. 실제 결제 및 상업적 이용으로 발생하는 모든 책임은 사용자에게 있으며, 사이트 운영 정책을 준수할 것을 권고합니다.
...
| 이미지 파일명 | 설명 | 조치 방법 |
| :--- | :--- | :--- |
| **`debug_no_login_btn.png`** | 메인 페이지에서 로그인 버튼을 찾지 못했을 때 찍힙니다. | 쿠팡 메인 UI가 변경되었는지 확인하거나, CSS 선택자를 수정해야 합니다. |
| **`error_no_product.png`** | 검색 결과에서 지정한 상품을 찾지 못했을 때 찍힙니다. | `TARGET_PRODUCT` 검색어를 더 정확하게 변경하거나, `TARGET_KEYWORD`가 목록에 노출되는지 확인하세요. |
| **`error_buy_now.png`** | 상세 페이지에서 '바로구매' 버튼을 찾지 못했을 때 생성됩니다. | 상품이 품절 상태인지, 혹은 UI가 로딩되지 않았는지 확인하세요. |
| **`error_pay.png`** | 주문/결제 화면에서 '결제하기' 버튼을 찾지 못했을 때 생성됩니다. | 결제 수단이 등록되어 있는지 확인하세요. |
| **`current_keypad.png`** | 결제 비밀번호 입력 화면의 키패드 부분을 캡처한 원본입니다. | 이 이미지가 잘렸거나 흐리게 나오는지 확인하세요. |
| **`debug_keypad_matches.png`** | `current_keypad.png` 위에서 템플릿 매칭 결과를 시각화(빨간 박스+점수)한 이미지입니다. | 매칭 점수가 낮다면 `templates/` 이미지를 새로 캡처해서 교체해야 합니다. |
